...
{-# LANGUAGE CPP, RecordWildCards, OverloadedStrings, PackageImports #-}

module Debug.Hoed.GHoed
where


import Paths_Hoed_extras(getDataDir,version)

import qualified Prelude
import Prelude hiding(Right)
import Debug.Hoed.Render
import Debug.Hoed.Serialize
import "Hoed" Debug.Hoed hiding (judge)
import Debug.Hoed.Utils
import qualified "Hoed" Debug.Hoed as Prop
...
import Graphics.UI.Threepenny (jsPort, jsStatic, startGUI,defaultConfig, Window, UI, (#), (#+), (#.), string, on,get,set)
...
ghoedExtra :: HoedExtrasArgs -> HoedAnalysis -> IO ()
ghoedExtra args h = case jshood args of
    None -> return ()
    View -> ghoedWith (options args) h
    Deploy -> return ()

runGHoedO :: IO a -> IO ()
runGHoedO = runGHoedOwith defaultHoedOptions

runGHoedOwith :: HoedOptions -> IO a -> IO ()
runGHoedOwith options program = do
    h <- runO' options program
    ghoedWith options h

ghoedWith :: HoedOptions -> HoedAnalysis -> IO ()
ghoedWith options h = do
    hoedTraceInfo <- traceInfo (verbose options) (hoedTrace h)
    let hoedEventForest = mkEventForest (hoedTrace h)
    debugSession (hoedTrace h) hoedTraceInfo (hoedCompTree h) hoedEventForest []
...
guiObserve compTreeRef currentVertexRef = do
       ...
       let slices' = sort $ map (stmtLabel . vertexStmt) . filter (not . isRootVertex) $ vs
           ...
           spans = map (\(c,lbl) -> span $ show c ++ " " ++ T.unpack lbl) $ zip (map count slices) slices
...
testCurrent compTreeRef traceRef ps status currentVertexRef compStmt handlerRef nextRef= do
  ...
  where
  onNoProps cv = do
    UI.element status # set UI.text ("Cannot test: no propositions associated with function " ++ (T.unpack . stmtLabel . vertexStmt) cv ++ "!")
    return ()
  onJudge =
    advance AdvanceToNext status compStmt Nothing Nothing currentVertexRef compTreeRef nextRef
