...
module Tarefa3_2017li1g183 where

import LI11718
import Tarefa2_2017li1g183
...
validaCarro tab carro = (not $ derrete tab (posicao carro))

ponto2Pos (i,j) = (round i, round j)
...
colideLocalAcc ps m v (a,b) (i,j) | morto = (ps++[a,b],Nothing)
                                  | colideInterno = colideLocalAcc (ps++[a]) m iVel iPos (i+1,j)  -- se houverem colisões internas (diagonais), da prioridade e volta a aplicar novo vector
