...
glossTempo t m = m { jogo = j', timer = t + timer m
  ...
  where (j',mrts,btts) = atualizaEMovimenta (float2Double t) (jogo m) (voltas m) as'
        ...
        whereAreWe = map (whereAmI (jogo m) 0) [0..njogadores-1]
        voltasI n | mxp-1 == snd ((voltas m)!!n) = (1 + fst ((voltas m)!!n),0)
                  | otherwise = (fst (voltas m!!n),nxt)
            where (nxts,mxp) = whereAreWe!!n
                  nxt = maximumNote "glossTempo" $ filter (\k -> k < 4 + snd ((voltas m)!!n)) nxts
        as1 = take botnunoid (acoes m)++(botnuno:drop (botnunoid+1) (acoes m))
        as2 = take bothugoid (acoes m)++(bothugo:drop (bothugoid+1) as1)
        botnuno = decide (float2Double t) m botnunoid
        botnunoid = 3
        bothugo = Hugo.bot (float2Double t) (jogo m) bothugoid
        bothugoid = 2
...
joga i j = do    
  ...
  where
    ...
    s = tamanho / 100
    ini@(Mapa p m) = mapas5!!i --mapas16!!i
    prp = pisos!!j
