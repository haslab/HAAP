...
isOpaque :: Mapa -> Pos -> Bool
isOpaque m p = isTijolo m p || isPowerUp m p
...
avancaTempo :: (Mapa,Burned) -> Int -> ((Mapa,Burned),[Word8])
avancaTempo (mapa,burned) ticks = avancaTempoWalls (mapa,burned) closedwalls
    where
    dim = boardDim $ board mapa
    -- calcula paredes fechadas
    nwalls = closedBlocks dim (ticks-1)
    closedwalls = closeParedes dim nwalls

avancaTempoWalls :: (Mapa,Burned) -> [Pos] -> ((Mapa,Burned),[Word8])
avancaTempoWalls (mapa,burned) closedwalls = ((mapa4,burned'),deadplayers)
    where
    dim = boardDim $ board mapa
    ...
    (mapa2,deadbombs) = Writer.runWriter $ modifyBombs mapa1 $ \bs -> do
        let bs' = Map.map (\b -> b { bTime = pred $ bTime b }) bs
        let (dead,others) = Map.partition ((==0) . bTime) bs'
        Writer.tell dead
        return (others,return)
    (mapa3,burned') = Writer.runWriter $ explodeBombs False (Map.toList deadbombs) mapa2
    mapa4 = mapa3 { board = (board mapa3) { boardCaracol = if List.null closedwalls then 0 else succ $ boardCaracol $ board mapa3 } }

-- explode bombas e desenha chamas
explodeBombs :: Bool -> [(Pos,Bomb)] -> Mapa -> Writer Burned Mapa
explodeBombs noSelf [] m = return m
explodeBombs noSelf (b:bs) m = explodeBomb noSelf b m >>= explodeBombs noSelf bs
explodeBomb :: Bool -> (Pos,Bomb) -> Mapa -> Writer Burned Mapa
explodeBomb noSelf (p,b) m = drawFlames noSelf p 0 Nothing (bRadius b) m
drawFlames :: Bool -> Pos -> Word8 -> Maybe Char -> Word8 -> Mapa -> Writer Burned Mapa
...
drawFlames noSelf p@(Pos x y) i (Just dir) j m = cont wall m $ case (i==j,dir) of
  ...
  where
    cont b m f = if b then return m else f
    bo = board m
    dim = boardDim bo
    wall = isWall dim p $ closeParedes dim $ boardCaracol bo
    stop = isOpaque m p
drawFlame :: Pos -> Int -> Mapa -> Writer Burned Mapa
drawFlame p i m = do
    Writer.tell [(p,i)]
    modifyBombs m $ \bs -> do
...
-- apaga bombas a zero e items queimados (jogadores,tijolos,powerups)
cleanseMap :: [Pos] -> [Pos] -> Mapa -> (Mapa,[Word8])
cleanseMap burned walls m = Writer.runWriter $ do
...
tarefa4Interval :: Mapa -> Int -> Int -> Mapa
tarefa4Interval m start end
    | start < end = m
    | otherwise = tarefa4Interval (tarefa4 m start) (pred start) end

tarefa4 :: Mapa -> Int -> Mapa
tarefa4 m ticks = m2
    where
    --deadbombs = concatMap (Map.toList . Map.filter ((==0) . bRadius) ) (mBombs m)
    --burned = Writer.execWriter $ explodeBombs deadbombs m
    ((m1,burned1),_) = avancaTempo (m,[]) ticks
    (m2,deads) = cleanseMap (map fst burned1) [] m1
    m3 = m2 { players = Map.filterWithKey (\k v -> not $ List.elem k deads) $ players m2 }

tarefa4Walls :: Mapa -> [Pos] -> Mapa
tarefa4Walls m closedwalls = m2
    where
    --deadbombs = concatMap (Map.toList . Map.filter ((==0) . bRadius) ) (mBombs m)
    --burned = Writer.execWriter $ explodeBombs deadbombs m
...
mutations :: [Mutation]
mutations = [Inverse,DoesNotRecognize 'L',DoesNotRecognize 'R',DoesNotRecognize 'U',DoesNotRecognize 'D',DoesNotRecognize 'B'
